...
public class BHBotConfigurationFactory extends ConfigurationFactory {
    ...
    private static Configuration createConfiguration(final String name, ConfigurationBuilder<BuiltConfiguration> builder) {
        ...
        stdOutBuilder.add(builder.newLayout("PatternLayout")
                .addAttribute("pattern", "%d{HH:mm:ss.SSS} %highlight{%level}{FATAL=bg_red, ERROR=red, WARN=yellow, INFO=green, DEBUG=blue} - %msg%n"));
        ...
        stdErrBuilder.add(builder.newLayout("PatternLayout").addAttribute("pattern", "%style{%d{HH:mm:ss.SSS} %highlight{%level}{FATAL=bg_red, ERROR=red, WARN=yellow, INFO=green, DEBUG=blue} - %msg%n}{red}"));
